...
class Moderation(commands.Cog):
	...
	async def user(self, ctx, member: discord.Member = None):
		...
		embed.add_field(name="Created at:", value=member.created_at.strftime("%d/%m/%y %H:%M %p GMT"),
						inline=False)
		embed.add_field(name="Joined at:", value=member.joined_at.strftime("%d/%m/%y %H:%M %p GMT"), inline=False)
	...
	async def insert_in_muted(self, user_id: int, role_id: int):
		...
		await mycursor.execute("INSERT INTO MutedMember (user_id, role_id) VALUES (%s, %s)", (user_id, role_id))
		await db.commit()
