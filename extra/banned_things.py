chat_filter = [
	'anal', 'anus','asshat','asshole','ballsack','bastard','bitch','b1tch','biatch','blowjob','bollock','bollocks','boner','boobs','buttplug','clitoris','cock','c0ck','cunt','dick','dildo','dyke','erection','fag','faggot','fagot','feck','fucker','fuckers','fucks','f4ck1ng','felching','fellate','fellatio','fudgepacker','genitals','jizz','bellend','knobhead','labia','masturbate','muff','negro','nigger','niggers','n1ggers','nigga','niggas','penis','pen1s','pube','pussy','pu55y','retarded','scrotum','semen','smegma','spunk','tit','tits','titty','tranny','trannies','trann1es','tosser','twat','vagina','wank','whore',
	'connard','connasse','pute','nègre','négresse','salope',
	'cago','chupamela','chupemela','concha','conchetumadre','coño','culo','escoria','follame','follar','fracasado','hijo perra','hija perra','idiota','joder','malparido','mamamela','marcia','mierda','nalga','parido','pelotudo','pija','pito','prostituta','puta','puto','sabrosa','sexo','vaginal','verga','violar','zorra',
	'allahu akbar','allah','الله أكبر','مقاطعة','Orospu çocuğu','İtoğluit','Piç kurusu','orospu çocuğu oç','aptal','gerizekalı','ananı sikim','siktir','salak','amına koyim',
	'arschloch','bastarde','ficker','fotze','hure','hurensohn','luder','Scheiße ','schlampe','schwuchtel','wichser','zicke', 'allah'
]


website_filter = [
	' https://giant.gfycat.com/WeirdShadowyGoa.mp4'
]